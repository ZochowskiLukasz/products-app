...
import soft.dev.academy.productsapp.repository.specification.ProductSpecification;
...
public class ProductServiceImpl implements ProductService {
    ...
    public List<ProductDto> findAll() {
        return StreamSupport.stream(
                productRepository.findAll().spliterator(), true)
    ...
    public List<ProductDto> findByNameAndType(String name, String type) {
        ...
        return StreamSupport.stream(
                productRepository.findAll(
                        new ProductSpecification(filterProduct)).spliterator(),
                true)
                .map(product -> productDtoConverter.convert(product))
                .collect(Collectors.toList());
    }
    ...
    public List<ProductDto> findByName(String name) {
        return StreamSupport.stream(
                productRepository.findByName(name).spliterator(), true)
                .map(product -> productDtoConverter.convert(product))
                .collect(Collectors.toList());
    }
}
